/**
 * Intake routes — implemented in phase C2.
 *
 * Currently answers 501 with the contract's failure envelope so a frontend built
 * against docs/CONTRACT.md fails legibly. Phase C2 REPLACES THIS FILE's handlers;
...
const notYet = (what) => (req, res) => {
  ...
  res.status(501).json({
    success: false,
    error: `${what} is implemented in phase C2 and is not available yet`,
    requestId: req.requestId,
  });
};

intakeRouter.post('/api/crm/intake', notYet('intake'));
intakeRouter.get('/api/crm/intake/:jobId/status', notYet('intake-status'));
